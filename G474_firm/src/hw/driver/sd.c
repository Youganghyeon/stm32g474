...
bool sdInit(void)
{
  ...
  hsd.Init.ClockDiv              = SDIO_TRANSFER_CLK_DIV;//0
...
{
  bool ret=false;
  sd_info_t *sd_p_info = (sd_info_t *)p_info;
  HAL_SD_CardInfoTypeDef sd_card;

  if(is_init==true)
  {
    HAL_SD_GetCardInfo(&hsd, &sd_card);

    sd_p_info->card_type = sd_card.CardType;
    sd_p_info->card_version = sd_card.CardVersion;
    sd_p_info->card_class= sd_card.Class;
    sd_p_info->rel_card_Add=sd_card.RelCardAdd;
    sd_p_info->block_numbers=sd_card.BlockNbr;
    sd_p_info->block_size= sd_card.BlockSize;
    sd_p_info->log_block_numbers=sd_card.LogBlockNbr;
    sd_p_info->log_block_size=sd_card.LogBlockSize;
    sd_p_info->card_size =  (uint32_t)((uint64_t)sd_p_info->block_numbers * (uint64_t)sd_p_info->block_size / (uint64_t)1024 / (uint64_t)1024);
    ret=true;
  }
  return ret;
}
...
bool sdReadBlocks(uint32_t block_addr, uint8_t *p_data, uint32_t num_of_blocks, uint32_t timeout_ms)
{
  ...
  if(HAL_SD_ReadBlocks_DMA(&hsd, p_data, block_addr, num_of_blocks)==HAL_OK)
  {
    pre_time = millis();
...
bool sdWriteBlocks(uint32_t block_addr, uint8_t *p_data, uint32_t num_of_blocks, uint32_t timeout_ms)
{
  ...
  is_tx_done=false;
  uint32_t pre_time;
  if(HAL_SD_WriteBlocks_DMA(&hsd, p_data, block_addr, num_of_blocks)==HAL_OK)
  {
    pre_time = millis();
...
static void cliSd(cli_args_t* args)
{
  ...
  {
    uint32_t number;
    uint32_t buf[512/4];  // uint32_t ->4byte. 1 block byte size = 512/4;
    number = (uint32_t)(args->getData(1));
    if(sdReadBlocks(number, (uint8_t*)buf, 1, 100)==true)
    {
      for(int i=0; i<512/4; i++)
      {
        cliPrintf(" %d:%04d block: 0x%08X \n", number, i*4, buf[i]);
      }
    }
    else
    {
